...
void *xmalloc_debug(const char *file, const char *func, int line, sys_size_t size) {
  ...
  if (ptr) {
    debug_full(file, func, line, DEBUG_TRACE, "MEM", "memory new %p %u", ptr, (unsigned int)size);
    sys_memset(ptr, 0, size);
  } else {
    debug_full(file, func, line, DEBUG_ERROR, "MEM", "memory new error %u", (unsigned int)size);
  }
...
void *xrealloc_debug(const char *file, const char *func, int line, void *ptr, sys_size_t size) {
  ...
  if (ptr2) {
    ...
    debug_full(file, func, line, DEBUG_TRACE, "MEM", "memory new %p %u", ptr2, (unsigned int)size);
  } else {
    if (size) {
      debug_full(file, func, line, DEBUG_ERROR, "MEM", "memory new error %u", (unsigned int)size);
    } else {
...
void *xmemcpy_debug(const char *file, const char *func, int line, void *dest, const void *src, sys_size_t n) {
  ...
  if (dest) {
    debug_full(file, func, line, DEBUG_TRACE, "MEM", "memory memcpy %p %p %u", dest, src, (unsigned int)n);
...
void *xmemset_debug(const char *file, const char *func, int line, void *s, int c, sys_size_t n) {
  ...
  if (s) {
    debug_full(file, func, line, DEBUG_TRACE, "MEM", "memory memset %p %d %u", s, c, (unsigned int)n);
    r = sys_memset(s, c, n);
  } else {
