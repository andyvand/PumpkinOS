...
#elif defined(_MSC_VER) && (defined(_M_X64_) || defined(_X64_) || defined(_M_X86) || defined(_X86_))
...
static uint8_t cpuPrvArmAdrMode_5(struct ArmCpu *cpu, uint32_t instr, uint32_t* addBeforeP, uint32_t* addAfterP, uint8_t *optionValP)
{
	...
	*optionValP = 0;
	...
	if (!(reg & ARM_MODE_5_IS_OPTION)) {
		...
		if (!(instr & 0x00800000UL)) {
			*addBeforeP = (uint32_t)-((int32_t)*addBeforeP);
			*addAfterP = (uint32_t)-((int32_t)*addAfterP);
		}
	}
	return reg;
}
...
#ifdef _MSC_VER
	if (UIntAdd((unsigned int)a, (unsigned int)b, (unsigned int *)&c) != S_OK) {
		return 1;
	}
...
#ifdef _MSC_VER
	if (UIntSub((unsigned int)a, (unsigned int)b, (unsigned int *)&c) != S_OK) {
		return 1;
	}
...
{
	return (sign < 0) ? ((int32_t)-0x80000000) : ((int32_t)0x7fffffff);
}
		...
		case 1:		//data processing immediate shift, register shift and misc instrs and mults
			if ((instr & 0x00000090UL) == 0x00000090) {		//multiplies, extra load/stores (table 3.2)
				...
				mul64:				
							...
							cpuPrvSetRegNotPC(cpu, (instr >> 12) & 0x0F, (uint32_t)res64);
							cpuPrvSetRegNotPC(cpu, (instr >> 16) & 0x0F, (uint32_t)(res64 >> 32));
...
data_processing:							//data processing
			{
				...
				switch ((instr >> 21) & 0x0F) {
...
#ifdef _MSC_VER
							cOut = (int)!(UIntSub(op1, op2, &res) != S_OK);
#else
...
#ifdef _MSC_VER
							cOut = (int)!(UIntSub((unsigned int)op1, (unsigned int)op2, (unsigned int *)&res) != S_OK);
#else
...
#ifdef _MSC_VER
							cOut = (int)(UIntAdd((unsigned int)op1, (unsigned int)op2, (unsigned int *)&res) != S_OK);
#else
					...
					case 5:			//ADC
						op1 = cpuPrvGetReg(cpu, (instr >> 16) & 0x0F, wasT, specialPC);
						res64 = (uint64_t)op1 + op2 + (cpu->C ? 1 : 0);
						res = (uint32_t)res64;
					...
					case 6:			//SBC
						op1 = cpuPrvGetReg(cpu, (instr >> 16) & 0x0F, wasT, specialPC);
						res64 = (uint64_t)op1 - op2 - (cpu->C ? 0 : 1);
						res = (uint32_t)res64;
					...
					case 7:			//RSC
						op1 = cpuPrvGetReg(cpu, (instr >> 16) & 0x0F, wasT, specialPC);
						res64 = (uint64_t)op2 - op1 - (cpu->C ? 0 : 1);
						res = (uint32_t)res64;
					...
					case 10:		//CMP
						...
						cpu->V = cpuPrvSignedSubtractionOverflows(op1, op2, res);
#ifdef _MSC_VER
						cOut = (int)!UIntSub((unsigned int)op1, (unsigned int)op2, (unsigned int *)&res);
#else
						cOut = !__builtin_sub_overflow(op1, op2, &res);
#endif
						goto dp_flag_set;
...
//debug(1, "XXX", "cpuPrvCycleArm pc=0x%08X", pc);
  ...
  if (fetchPc >= 0x04000000) {
    ...
    uint32_t group = 0, function = 0, lr = 0;
	group = (fetchPc & 0x00F00000) >> 20;
    function = fetchPc & 0xFFFF;
    lr = cpu->regs[REG_NO_LR];
