...
void cpu_write_long(uint32_t address, uint32_t value) {
  ...
  if (state->write_long) {
	  state->write_long(address, value);
	  return;
  }
...
void encode_deviceinfo(uint32_t deviceInfoP, DeviceInfoType *deviceInfo) {
  if (deviceInfoP && deviceInfo) {
    ...
    m68k_write_memory_32(deviceInfoP + 16, deviceInfo->serDevPortInfoStr ? (uint32_t)((uint8_t *)deviceInfo->serDevPortInfoStr - ram) : 0);
  }
}
...
void decode_notify(uint32_t notifyP, SysNotifyParamType *notify) {
  ...
  if (notifyP && notify) {
    ...
    userDataP = m68k_read_memory_32(notifyP + 12);
...
void encode_notify(uint32_t notifyP, SysNotifyParamType *notify) {
  if (notifyP && notify) {
    ...
    userDataP = notify->userDataP ? (uint32_t)((uint8_t *)notify->userDataP - ram) : 0;
...
Int16 CallCompareFunction(UInt32 comparF, void *e1, void *e2, Int32 other) {
  ...
  if ((p = pumpkin_heap_alloc(argsSize, "CallCompareFunction")) != NULL) {
    uint8_t *ram = pumpkin_heap_base();
    a =(uint32_t)(p - ram);
    m68k_write_memory_32(a, (uint32_t)((uint8_t *)e1 - ram));
    m68k_write_memory_32(a + 4, (uint32_t)((uint8_t *)e2 - ram));
...
void CallListDrawItem(UInt32 addr, Int16 i, RectangleType *rect, char **text) {
  ...
  if ((p = pumpkin_heap_alloc(argsSize + sizeof(RectangleType), "CallList")) != NULL) {
    ...
    m68k_write_memory_32(a + sizeof(int16_t) + sizeof(uint32_t), text ? (uint32_t)((uint8_t *)&text[0] - ram) : 0);
...
void CallTableDrawItem(UInt32 addr, TableType *tableP, Int16 row, Int16 column, RectangleType *rect) {
  ...
  if ((p = pumpkin_heap_alloc(argsSize + sizeof(RectangleType), "CallTable")) != NULL) {
    uint8_t *ram = pumpkin_heap_base();
    a = (uint32_t)(p - ram);
    m68k_write_memory_32(a, (uint32_t)((uint8_t *)tableP - ram));
...
Err CallTableLoadData(UInt32 addr, TableType *tableP, Int16 row, Int16 column, Boolean editable, MemHandle *dataH, Int16 *dataOffset, Int16 *dataSize, FieldPtr fld) {
  ...
  if ((p = pumpkin_heap_alloc(argsSize +  sizeof(MemHandle) + 2*sizeof(Int16), "CallTable")) != NULL) {
    ...
    m68k_write_memory_32(a + sizeof(uint32_t) + 3*sizeof(int16_t) + 3*sizeof(uint32_t), (uint32_t)((uint8_t *)fld - ram));
...
uint32_t arm_native_call_pce(uint32_t code, uint32_t userData) {
  ...
  sysAddr = (uint32_t)((uint8_t *)state->systable - ram);

  retAddr = (uint32_t)(state->armReturnAddr - ram);
  callAddr = (uint32_t)(state->armCall68KAddr - ram);
  emulStateAddr = (uint32_t)(state->armEmulState - ram);
  stackAddr = (uint32_t)(state->armStack - ram);
...
static emu_state_t *emupalmos_new(void) {
  ...
  if ((state = xcalloc(1, sizeof(emu_state_t))) != NULL) {
	...
	state->systable[1] = (uint32_t)((uint8_t *)dalFunctions - ram);
	state->systable[2] = (uint32_t)((uint8_t *)bootFunctions - ram);
	state->systable[3] = (uint32_t)((uint8_t *)uiFunctions - ram);
...
uint32_t emupalmos_main(uint16_t launchCode, void *param, uint16_t flags) {
  ...
  if (amdc0 && amdd0) {
	...
	paramBlockStart = paramBlock ? (uint32_t)(paramBlock - ram) : 0;
	...
	sysAppInfoStart = (uint32_t)(sysAppInfo - ram);
	...
	put4l((uint32_t)((uint8_t *)hAmdc0 - ram), sysAppInfo, 12); // codeH
	...
	arm_native_call_sub((uint32_t)(amdc0 - ram), (uint32_t)(amdd0 - ram), 0, 0, 0, 0);
  ...
  if (code1) {
	  ...
	  m68k_write_memory_32(sysAppInfoStart + 12, (uint32_t)((uint8_t *)hCode1 - ram)); // codeH
      ...
      if (data0) {
        ...
        data0_xrefs = i;
        if (data0_xrefs < (int32_t)(data0Size - 12)) {
        ...
        code1_xrefs = i;
        if (code1_xrefs < (int32_t)(data0Size - 12)) {
  ...
  if (readProc && dbID) {
    ...
    if ((buf = pumpkin_heap_alloc(bufferSize, "ExgDBReadARM")) != NULL) {
      ...
      if ((err = VFSFileOpen(1, pathName, vfsModeCreate | vfsModeWrite, &fileRef)) == errNone) {
        size = (uint32_t)(buf - (uint8_t *)pumpkin_heap_base());
        sizeP = (uint32_t *)buf;
        *sizeP = DBREAD_SIZE;
