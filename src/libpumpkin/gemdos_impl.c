...
uint16_t Tgetdate(void) {
  ...
  d = dateTime.day | (dateTime.month << 5) | ((dateTime.year - 1980) << 9);
...
uint32_t Tgettime(void) {
  ...
  seconds = TimGetSeconds();
  TimSecondsToDateTime(seconds, &dateTime);
  t = dateTime.second | (dateTime.minute << 5) | (dateTime.hour << 11);
...
int32_t Fread(int16_t handle, int32_t count, void *buf) {
  ...
  if (buf) {
    if (handle == 0) {
      ...
      r = (int32_t)plibc_read(handle, buf, count);
    } else {
...
int32_t Fwrite(int16_t handle, int32_t count, void *buf) {
  ...
  if (buf) {
    if (handle == 1) {
      ...
      r = (int32_t)plibc_write(handle, buf, count);
    } else if (handle > 3) {
      r = (int32_t)plibc_write(handle, buf, count);
    } else {
...
int32_t Fseek(int32_t offset, int16_t handle, int16_t seekmode) {
  ...
  return (int32_t)plibc_lseek(handle, offset, whence);
}
...
int32_t Mshrink(void *block, int32_t newsiz) {
  ...
  if (data->heap == NULL) {
    data->heapStart = (uint32_t)((uint8_t *)block - data->memory + newsiz);
...
static int32_t fsfirst_fill(tos_data_t *data, char *pattern, char **list, int32_t i, int32_t n, uint32_t dta) {
  ...
  if (VFSFileOpen(data->volume, list[i], vfsModeRead, &fileRef) == errNone) {
    ...
    m68k_write_memory_8(dta + 21, (uint8_t)attr);
...
int32_t Fsfirst(char *pat, int16_t attr) {
  ...
  if (pat) {
    ...
    for (i = (int32_t)(sys_strlen(pattern)-1); i >= 0; i--) {
      if (pattern[i] != ' ') break;
      pattern[i] = 0;
    }
