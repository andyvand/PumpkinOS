...
TIMI_EXPORT void mid_song_start(MidSong *song)
{
  song->playing = 1;
...
TIMI_EXPORT void mid_song_seek(MidSong *song, uint32 ms)
{
  skip_to(song, (ms * (song->rate / 100)) / 10);
}

TIMI_EXPORT uint32 mid_song_get_total_time(MidSong *song)
{
...
TIMI_EXPORT uint32 mid_song_get_time(MidSong *song)
{
...
TIMI_EXPORT char *mid_song_get_meta(MidSong *song, MidSongMetaId what)
{
  return (what < 0 || what >= MID_META_MAX)? NULL : song->meta_data[what];
}

TIMI_EXPORT size_t mid_song_read_wave(MidSong *song, sint8 *ptr, size_t size)
{
...
TIMI_EXPORT void mid_song_set_volume(MidSong *song, int volume)
{
  int i;
