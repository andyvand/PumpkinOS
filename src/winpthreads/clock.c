 ...
 */
#ifdef _WIN32_WCE
BOOL GetSystemTimeAdjustmentCE(
	PDWORD lpTimeAdjustment,
	PDWORD lpTimeIncrement,
	PBOOL  lpTimeAdjustmentDisabled
	) {
	if (!lpTimeAdjustment || !lpTimeIncrement || !lpTimeAdjustmentDisabled) return FALSE;

	// Windows CE heeft meestal een vaste tick (bijv. 10ms = 100.000 honderd-nanoseconden)
	*lpTimeIncrement = 100000;
	*lpTimeAdjustment = 100000;
	*lpTimeAdjustmentDisabled = TRUE; // Geen actieve bijstelling in de Win32-laag
...
#ifdef _WIN32_WCE
			(void) GetSystemTimeAdjustmentCE(&timeAdjustment, &timeIncrement, &isTimeAdjustmentDisabled);
#else
...
{
    struct _timespec64 tp64 = { tp->tv_sec, tp->tv_nsec};
